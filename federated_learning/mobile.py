...
class Mobile():
    ...
    def update(self, data_required, energy_required):
        if (data_required != 0 and energy_required != 0):
            CPU_required = math.sqrt(
                energy_required * Mobile.ENERGY_OF_UNIT * 1.0 / (Mobile.TAU * Mobile.CPU_CYCLE_PER_UNIT * data_required)) / (0.4 * math.pow(10, 9))
