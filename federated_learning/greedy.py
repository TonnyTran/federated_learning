...
while (step < nb_steps):
    ...
    if env.MB1.CPU_shared != 0:
        data_required1 = min(int(round(energy_required1 * Mobile.ENERGY_OF_UNIT / (Mobile.TAU * Mobile.CPU_CYCLE_PER_UNIT * math.pow(env.MB1.CPU_shared * Mobile.CPU_UNIT, 2)))), Mobile.MAX_DATA)
    else:
        data_required1 = 0
    if env.MB2.CPU_shared != 0:
        data_required2 = min(int(round(energy_required2 * Mobile.ENERGY_OF_UNIT / (Mobile.TAU * Mobile.CPU_CYCLE_PER_UNIT * math.pow(env.MB2.CPU_shared * Mobile.CPU_UNIT, 2)))), Mobile.MAX_DATA)
    else:
        data_required2 = 0
    if env.MB3.CPU_shared != 0:
        data_required3 = min(int(round(energy_required3 * Mobile.ENERGY_OF_UNIT / (Mobile.TAU * Mobile.CPU_CYCLE_PER_UNIT * math.pow(env.MB3.CPU_shared * Mobile.CPU_UNIT, 2)))), Mobile.MAX_DATA)
    else:
        data_required3 = 0
