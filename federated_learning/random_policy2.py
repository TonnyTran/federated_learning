...
sheet = workbook.add_sheet('Random')
...
while (step < nb_steps):
    data_required1 = random.randint(1, Mobile.MAX_DATA)
    data_required2 = random.randint(1, Mobile.MAX_DATA)
    data_required3 = random.randint(1, Mobile.MAX_DATA)
    if env.MB1.CPU_shared != 0 and env.MB1.energy != 0:
        ...
        if energy_required1 > 0:
            energy_required1 = random.randint(1, energy_required1)
    else:
        energy_required1 = 0
    ...
    if env.MB2.CPU_shared != 0 and env.MB2.energy != 0:
        ...
        energy_required2 = min(energy_required2, env.MB2.energy)
        if energy_required2 > 0:
            energy_required2 = random.randint(1, energy_required2)
    else:
        energy_required2 = 0
    ...
    if env.MB3.CPU_shared != 0 and env.MB3.energy != 0:
        ...
        energy_required3 = min(energy_required3, env.MB3.energy)
        if energy_required3 > 0:
            energy_required3 = random.randint(1, energy_required3)
